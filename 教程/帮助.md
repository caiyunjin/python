...
```py
# input()，接收用户输入的内容，类型为字符串，所以你输入数字类型也为字符串
a = int(input("请输入一个数字：")) # 将用户输入的字符串转int类型
b = int(input("请输入另一个数字：")) # 将用户输入的字符串转int类型
print(a + b)
```
...
a1 = 1
a2 = 2
if a1 >= a2: # 如果条件为True，执行下面代码
    print(a1, "大于等于", a2) 
else: # 相反为flase，执行下面代码
    print(a1, "小于", a2) # 1 >= 2为false，所以程序会输出这一条内容
...
if a1 >= a2:
    ...
    pass # 当下想不出要编写哪些执行代码，可用pass占位，方便以后添加，不然程序会报错
...
# ----------------------------------------------------------------------
# while 循环，当条件为真，会一直循环执行
# 定义变量 a 和 sum 的初始值
a = 1
sum = 0
while a <= 100: # 定义 a 的范围
    # 偶数和
...
for _ in range(10): # _ 代表临时变量
    print("python") # 结果为 输出10次python
...
函数是对数据与代码的封装,实现了代码的重复使用

函数分四类：

```
1.内置函数，如 list()、int()等
2.标准库函数，需要用 import 语句导入，如time、os
3.第三方库，需要下载，再用 import 语句导入
4.自己定义的函数
```
...
def calc(a, b):  # a、b 形式参数
    c = a + b
    return c # return 返回值 c
# 位置实参，按位置传递参数，也就是 a = 1，b = 2
res = calc(1,2) # 调用函数，把值赋给变量 res
print(res) # 打印 res，结果为 3

# 关键字实参，按关键字传递参数，也就是 a = 1，b = 2
res = calc(b = 1,a = 2)
print(res) # 结果为 3
...
# ----------------------------------------------------------------------
# 综合案例
# 定义全局变量money name，定义的变量在函数外为全局变量，在函数里的为局部变量
money = 5000000
name = None
# 要求客户输入姓名
name = input("请输入您的姓名：")
...
"""
# with（上下文管理器），不论什么原因跳出with语句，都能确保文件正确关闭，来达到释放资源的目的。不用 with 最后要用过 close() 关闭文件
with open("D:/test.txt", "r") as f:
    print(f.read()) # 读取全部内容

# 读取
file = open("D:/test.txt", "r") # 包含中文 file = open("D:/test.txt", "r"，encoding='utf-8')
print(file.readline())  # 每次读取一行，可用 while 循环读取全部行

print(file.readlines())  # readlines() 读取文件中的所有行，返回的是一个字符串列表
file.close()
...
```py
# 捕获异常并输出，异常是Python解释器检测到一个错误时，无法继续执行并出现了一些错误的提示
# try except，首先会执行try部分，如果try报错，就会执行except部分，如果try部分没有报错，程序就会跳过except部分执行
try:
    a = int(input("请输入第一个数"))
    b = int(input("请输入第二个数"))
    result = a / b
...
class Student:
    ...
    def say_hi(self): # 类中的函数称为方法，参数必须带 self
...
# ----------------------------------------------------------------------
# 封装
# 定义一个类，内含私有变量或私有方法，不能被外部调用，例：__变量名，__方法()
class Phone:
...
```py
# 装饰器本质上是一个函数，返回值是一个函数对象。通过利用装饰器，我们可以让其他函数在不做任何代码改动的情况下增加额外的功能，同时也能够让代码更加简洁
# 装饰器的一般写法（闭包）
def outer(func):
    def inner():
        print("我睡觉了")
...
```py
# 工厂模式提供了创建对象的最佳方法之一。在工厂模式中，对象的创建不向客户端公开逻辑，并使用通用接口引用新创建的对象
class Person:
    pass
...
```py
# socket通常被称作"套接字"，用于描述IP地址和端口，是一个通信链的句柄,应用程序通常通过"套接字"向网络发出请求或者应答网络请求
# 服务端和客户端代码分别写在不同py文件中，先启动服务端再启动客户端
# ----------------------------------------------------------------------
# 服务端
import socket
